...
function findElementGreaterThan(arr, num) {
  if (!Array.isArray(arr)) {  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо undefined
      ...
      let result = arr.find(elem => elem > num);  // Використовуємо метод find для пошуку першого елементу, який є більшим за задане число num.
      if (result == undefined) {  // Перевіряємо чи є елемент який задовільняє умову
        return `Числа яке задовільняе умову не знайдено`  // Якщо ні повертаємо рядок Числа яке задовільняе умову не знайдено
      } else {
        return result  //Якщо так повертаємо елемент
      };
    }
}
...
function findLastEvenNumber(arr) {
  if (!Array.isArray(arr)) {
    ...
    let result = arr.findLast(elem => elem % 2 === 0);  // За допомогою метода findLast знаходимо останнє парне число в масиві.
    if (result == undefined) {
      return `Парних чисел не знайдено`  // Повертаємо знайдене число або 'Парних чисел не знайдено', якщо таке число не знайдено.
    } else {
      return result
    }; 
  }
...
function getSubArrayAndConvertToString(arr, startIdx) {
  if (!Array.isArray(arr)) {
    ...
    return arr.slice(startIdx).join(' ')  // Повертаємо рядок.
  }
}
...
function removeAndDouble(arr, startIdx) {
  if (!Array.isArray(arr)) {
    ...
    return arr.map(elem => elem * 2);   // За допомогою методу map() подвоюємо кожен елемент масиву.
  }
...
function sumAndPrint(arr) {
  if (!Array.isArray(arr)) {
    ...
    arr.forEach(elem => console.log(elem));  // За допомогою методу forEach() виводимо кожен елемент масиву в консоль.
    return arr.reduce((accum, value) => accum + value, 0);   // За допомогою методу reduce() обчислюємо суму елементів масиву.
  }
...
return   arr.flat().map(elem => elem ** 2).filter((elem) => elem > threshold);
  }
...
function getLastElementsSumAndPrint(arr, n) {
  if (!Array.isArray(arr)) {
    ...
    arr.slice(-n).forEach((elem) => console.log(elem))  // За допомогою методу slice() вибираємо останні n елементів масиву.
    return  arr.slice(-n).reduceRight((accum, value) => accum + value, 0);  // За допомогою методу reduceRight() обчислюємо суму останніх n елементів масиву.
  }
...
function expandArrayByN(arr, n) {
  if (!Array.isArray(arr)) {
    ...
    return arr.flatMap((elem) => Array.of(elem, elem, elem));
  }
